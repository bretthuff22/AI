...
PathFollowingBehavior::PathFollowingBehavior(Agent* pAgent, float weight)
	: SteeringBehavior(pAgent, weight)
{

}

SVector2 PathFollowingBehavior::Update(float deltaTime)
{
	...
	if (distSq < velocitySq)
	{
		...
		else if (distSq <= stopRadiusSq)
		{
			...
			if (!mDestinations.empty())
			{
				mpAgent->SetDestination(mDestinations.front());
			}
		}
	}
